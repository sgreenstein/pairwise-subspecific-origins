...
CHROMO_SIZES = [    197195432, 181748087, 159599783, 155630120, 152537259,
                    ...
                    91744698,      16299 ] # these are mm9 cooords
...
def make_source_matrix(strain_name, chromosomes):
    ...
    np.save(os.path.join("intervals", strain_name + '_intervals.npy'), intervals)
    np.save(os.path.join("intervals", strain_name + '_sources.npy'), sources_new)
...
def rgb_to_subspecies(file_list, suffix='_ss_names'):
    ...
    for file_name in file_list:
        ...
        this_file = file_name[:dot_pos] + suffix + file_name[dot_pos:]
        new_filenames.append(this_file)
        with open(file_name) as in_file, open(this_file, 'w+') as out_file:
            first = 0
            for row in in_file:
                if not first:
                    out_file.write(row)
                    ...
                    row = row.strip()
                    # skip regions with ambiguous origin (mostly applies to wild-caught mice)
                    if not (row.endswith(",255 0 0") or row.endswith(",0 255 0") or row.endswith(",0 0 255")):
                        continue
                    else:
                        row = row.replace('255 0 0', 'mus')
                        row = row.replace('0 255 0', 'cas')
    ...
    for filename in file_list:
        with open(filename) as csvfile:
            ...
            for row in reader:
                ...
                last_species = 0 if not intervals else intervals[-1][0]
                print row
                if last_species != SPECIES_TO_INT[row[SUBSPECIES]]:
                ...
                lastEnd = row[END]
                last_species = SPECIES_TO_INT[row[SUBSPECIES]]
...
def source_frequencies(strain_names, in_path = ""):
    ...
    :param in_path: where to look for pre-computed intervals from preprocess()
    """
...
def calc_genomic_area(counts, intervals):
    '''
    ...
    print intervals[:10]
    print (areas[ range(0,10),: ])[ :,range(0,10) ]

    areas_masked = OrderedDict()
    # get denominator for genomic area
    genome_len = sum(float(x)/1.0e6 for x in CHROMO_SIZES)-(3.0*20.0)
    denom = genome_len**2
...
def list_available_strains(globber):
    '''
    Given path to a databse of pre-computed subspecies origin intervals,
    list the strains that are available.
    :param globber: file pattern for finding strains, possibly including wildcards
    '''
    sources = []
    for s in glob.glob(globber + "_sources.npy"):
        sources.append( os.path.basename(s).replace("_sources.npy","") )

    intervals = []
    for s in glob.glob(globber + "_intervals.npy"):
...
def main():
    ...
    # processed_files = rgb_to_subspecies(["subspecific_origins.csv"])
    # strain_names = preprocess(processed_files)
    # print strain_names
    # strain_names = preprocess(["cc.subspecies.csv"])
    #strain_names = preprocess(["test.csv"])
    #print strain_names
    #sys.exit(1)

    # the following you'll need to run multiple times
    # for any subset you want of strain names, run something like:
    strain_names = list_available_strains("intervals/STU*")[0:4]

    sys.stderr.write("Computing pairwise subspecies origins for these strains:\n")
    sys.stderr.write("\t-- {}\n".format("\n\t-- ".join(strain_names)))

    pairs = source_frequencies(strain_names, in_path = "intervals/")
    combos = OrderedDict()
    for s1, i1 in SPECIES_TO_INT.iteritems():
        for s2, i2 in SPECIES_TO_INT.iteritems():
            combos[ s1 + "_" + s2 ] = i1+i2
            combos[ s2 + "_" + s1 ] = -1*(i1+i2)

    sys.stderr.write("\nSumming up...\n")
    areas = calc_genomic_area(pairs[0], pairs[1])
