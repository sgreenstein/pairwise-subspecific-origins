...
this_file = os.path.basename(__file__).replace('App.py', '')
...
def indexPage(form):
    ...
    avail_strains = tl.list_available_strains()
    for text, value, strains in STRAIN_SETS:
        ...
        panel.add("""<select data-placeholder=%s id=%s multiple="multiple" class="chosen">""" % (text, value))
