...
var COLOR_CURVE_SCALE = 20; // how quickly the color increases in saturation
...
function flattenIndex(i, j) {
    return (i * chrom_offsets.length) + j;
}

function unflattenIndex(k) {
    return [Math.floor(k / chrom_offsets.length), k % chrom_offsets.length]
}
...
for (var i = 0; i < Math.pow(chrom_offsets.length, 2); i++) {
    chromo_pairs.push([]);
    unique_areas.push({});
}
prox_chrom = 0;
...
for (i = 0; i < data.length; i++) {
    d = data[i];
    while (d[PROX_START] > chrom_offsets[prox_chrom]) {
        prox_chrom++;
        dist_chrom = 0;
    }
    while (d[DIST_START] > chrom_offsets[dist_chrom]) {
        dist_chrom++;
    }
...
function scale(pos) {
    return (pos / chrom_offsets[chrom_offsets.length-1]) * chart_height;
}
...
chromo_rect.append("rect")
    ...
    .attr("fill", function(d, k) {
        ...
        var alpha = 2 / (1 + Math.exp(-COLOR_CURVE_SCALE*total_unique_area / chrom_area)) - 1;
        var whiteness = Math.round((1 - alpha) * 0x0000ff);
        max_rgb = max_rgb.map(function (c) { return Math.round(c * alpha)});
